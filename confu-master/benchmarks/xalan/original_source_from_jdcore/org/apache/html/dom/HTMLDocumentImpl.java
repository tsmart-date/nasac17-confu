  ...
  implements HTMLDocument
{
  ...
  public synchronized String getTitle()
  {
    ...
    Node localNode = localHTMLElement.getElementsByTagName("TITLE").item(0);
    NodeList localNodeList = localHTMLElement.getElementsByTagName("TITLE");
    if (localNodeList.getLength() > 0)
    {
      localNode = localNodeList.item(0);
  ...
  public Node cloneNode(boolean paramBoolean)
  {
    ...
    if (paramBoolean) {
      for (NodeImpl localNodeImpl = (NodeImpl)getFirstChild(); localNodeImpl != null; localNodeImpl = (NodeImpl)localNodeImpl.getNextSibling()) {
        localHTMLDocumentImpl.appendChild(localHTMLDocumentImpl.importNode(localNodeImpl, true));
      }
    }
