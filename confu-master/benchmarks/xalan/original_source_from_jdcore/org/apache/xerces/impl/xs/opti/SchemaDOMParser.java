  ...
  extends DefaultXMLDocumentHandler
{
    ...
    if (fInnerAnnotationDepth == -1) {
      for (int i = offset; i < offset + length; i++) {
        if (!XMLChar.isSpace(ch[i]))
        {
          ...
          fErrorReporter.reportError("http://www.w3.org/TR/xml-schema-1", "s4s-elt-character", new Object[] { str }, (short)1);
          break;
        }
      }
    } else {
    ...
    throws XNIException
  {
    if ((fGenerateSyntheticAnnotation) && (fAnnotationDepth == -1) && (uri == SchemaSymbols.URI_SCHEMAFORSCHEMA) && (localpart != SchemaSymbols.ELT_ANNOTATION) && (hasNonSchemaAttributes(paramQName, paramXMLAttributes)))
    {
      ...
      schemaDOM.startAnnotation(localQName1, paramXMLAttributes, fNamespaceContext);
      ...
      schemaDOM.startAnnotationElement(localQName2, paramXMLAttributes);
      schemaDOM.characters(new XMLString("SYNTHETIC_ANNOTATION".toCharArray(), 0, 20));
      schemaDOM.endSyntheticAnnotationElement(localQName2, false);
      schemaDOM.endSyntheticAnnotationElement(localQName1, true);
    ...
    throws XNIException
  {
    ...
    else
    {
      if ((uri == SchemaSymbols.URI_SCHEMAFORSCHEMA) && (fGenerateSyntheticAnnotation))
      {
        ...
        if ((bool1) && (!bool2))
        {
          String str = fNamespaceContext.getPrefix(SchemaSymbols.URI_SCHEMAFORSCHEMA);
          ...
          schemaDOM.startAnnotation(localQName1, fEmptyAttr, fNamespaceContext);
          ...
          schemaDOM.startAnnotationElement(localQName2, fEmptyAttr);
          schemaDOM.characters(new XMLString("SYNTHETIC_ANNOTATION".toCharArray(), 0, 20));
          schemaDOM.endSyntheticAnnotationElement(localQName2, false);
          schemaDOM.endSyntheticAnnotationElement(localQName1, true);
        }
