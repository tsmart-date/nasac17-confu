  ...
  implements XSAttributeGroupDefinition
{
  ...
  public void removeProhibitedAttrs()
  {
    ...
    for (int j = 0; j < fAttrUseNum; j++) {
      ...
      {
        i++;
        arrayOfXSAttributeUseImpl[(fAttrUseNum - i)] = fAttributeUses[j];
      }
    }
    int k = 0;
    if (i > 0)
    {
      for (int m = 0; m < fAttrUseNum; m++) {
        if (fAttributeUses[m].fUse != 2)
        {
          int n = 1;
          while ((fAttributeUses[m].fAttrDecl.fName != fAttrUseNum - i)].fAttrDecl.fName) || (fAttributeUses[m].fAttrDecl.fTargetNamespace != fAttrUseNum - i)].fAttrDecl.fTargetNamespace))
          {
            n++;
            if (n > i) {
              arrayOfXSAttributeUseImpl[(k++)] = fAttributeUses[m];
            }
          }
        }
      }
